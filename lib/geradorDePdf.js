...
var path = require("path"),
  Pdf = require("pdfkit"),
  diretorioDeFontes = path.join(__dirname, "./lib/fontes/code128.ttf"),
  timesNewRoman = path.join(__dirname, "./fontes/Times New Roman.ttf"),
  timesNewRomanNegrito = path.join(
    __dirname,
    "./fontes/Times New Roman Bold.ttf"
  ),
  timesNewRomanItalico = path.join(
    __dirname,
    "./fontes/Times New Roman Italic.ttf"
  ),
  timesNewRomanNegritoItalico = path.join(
    __dirname,
    "./fontes/Times New Roman Bold Italic.ttf"
  );

var barcode = require("./barcodeUtils");
...
var pdfDefaults = {
  ...
  tamanhoDoCodigoDeBarras: 20,
  corDoLayout: "black",
  larguraDaPagina: 595.44,
  alturaDaPagina: 841.68,
  creditos: "empresa desenvolvedora teste",
};
...
function forEachOwnProperty(object, iterator) {
  ...
  for (var property in object) {
    if (object.hasOwnProperty(property)) {
      if (
        iterator(property, object[property], current++ < propertiesLength) ===
        "break"
      ) {
        break;
      }
...
module.exports = function (danfe, outpuFile, args, callback) {
  ...
  var alturaDoBlocoFaturaDuplicatas = 0,
    pdf = new Pdf({
      bufferPages: true,
      margin: 0,
  ...
  function desenharPagina() {
    ...
    var alturaInicialDoQuadroDeItens = 383,
    ...
    normal(
      [
        ...
        danfe.DataDaEmissao,
        "- Valor Total:",
        danfe.ValorTotalDaNota,
        "- Destinatário:",
        danfe.Destinatario.Nome,
        "- Endereço:",
        danfe.Destinatario.Endereco.Logradouro,
      ]
        .join(" ")
        .toUpperCase(),
      1.5,
    ...
    normal(danfe.Numero, 476.6, 22, 110, "center", 10);
    normal(danfe.Serie, 476.6, 31.5, 110, "center", 10);

    italico("IDENTIFICAÇÃO DO EMITENTE", 1, 60, 238);

    var temLogotipo = danfe.Emitente.Logotipo,
    ...
    if (temLogotipo) {
      var caminhoDoLogotipo = danfe.Emitente.Logotipo;
      pdf.image(
        caminhoDoLogotipo,
    ...
    if (danfe.Emitente.Endereco.Logradouro) {
      normal(
        danfe.Emitente.Endereco.Logradouro,
        identificacaoDoEmitenteX,
        pdf.y - margemTopo + 2,
        identificacaoDoEmitenteLargura,
        "center",
    ...
    if (danfe.Emitente.Telefone) {
      jaAdicionouEspacamento = true;

      normal(
        "Telefone: " + danfe.Emitente.Telefone,
        identificacaoDoEmitenteX,
        pdf.y - margemTopo + 2,
        identificacaoDoEmitenteLargura,
        "center",
    ...
    if (danfe.Emitente.Email) {
      normal(
        "Email: " + danfe.Emitente.Email,
        identificacaoDoEmitenteX,
    ...
    normal(danfe.Numero, 241.2, 119.5, 98.5, "center", 10);
    normal(danfe.Serie, 241.2, 129.5, 98.5, "center", 10);

    //RECIBO
    titulo("DATA DE RECEBIMENTO", 1.5, 29, 97);
    titulo("IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR", 100.5, 29, 374);

    var codigoDeBarrasCodificado = danfe.ChaveDeAcesso;
    if (danfe.ChaveDeAcesso) {
      pdf
        .font("codigoDeBarras")
    ...
    campo(danfe.Protocolo.Codigo, 341.5, 158.4, 244);
    ...
    campo(danfe.Emitente.InscricaoEstadual, 1.5, 178.4, 192.5);
    ...
    campo(danfe.Emitente.RegistroNacional, 392.5, 178.4, 192.5);

    //SEGUNDO BLOCO
    secao("DESTINATÁRIO / REMETENTE", 1.5, 194);
    titulo("NOME / RAZÃO SOCIAL", 1.5, 202, 353.5);
    campo(danfe.Destinatario.Nome, 1.5, 210, 353.5, "left");

    titulo("CNPJ / CPF", 358, 202, 133.5);
    campo(danfe.Destinatario.RegistroNacional, 358, 210, 133.5);

    titulo("DATA DA EMISSÃO", 495, 202, 90);
    campo(danfe.DataDaEmissao, 495, 210, 90);
    ...
    campo(danfe.Destinatario.Endereco.Bairro, 276, 230, 119);

    titulo("CEP", 398, 222, 93);
    campo(danfe.Destinatario.Endereco.Cep, 398, 230, 93);

    titulo("DATA DA SAÍDA", 495, 222, 90);
    campo(danfe.DataDaEntradaOuSaida, 495, 230, 90);

    titulo("MUNICÍPIO", 1.5, 242, 272);
    campo(danfe.Destinatario.Endereco.Municipio, 1.5, 250, 272, "left");

    titulo("UF", 276, 242, 20);
    campo(danfe.Destinatario.Endereco.Uf, 276, 250, 20);

    titulo("FONE / FAX", 299, 242, 96);
    campo(danfe.Destinatario.Telefone, 299, 250, 96);

    titulo("INSCRIÇÃO ESTADUAL", 398, 242, 93);
    campo(danfe.Destinatario.InscricaoEstadual, 398, 250, 93);
    ...
    campo(danfe.Impostos.BaseDeCalculoDoIcms, 1.5, 278, 84, "right");

    titulo("VALOR DO ICMS", 89, 270, 84);
    campo(danfe.Impostos.ValorDoIcms, 89, 278, 84, "right");

    titulo("BASE DE CÁLC. ICMS S.T.", 177, 270, 84);
    campo(danfe.Impostos.BaseDeCalculoDoIcmsSt, 177, 278, 84, "right");

    titulo("VALOR DO ICMS SUBST.", 265, 270, 84);
    campo(danfe.Impostos.ValorDoIcmsSt, 265, 278, 84, "right");

    titulo("VALOR IMP. IMPORTAÇÃO", 353, 270, 84);
    campo(danfe.Impostos.ValorDoImpostoDeImportacao, 353, 278, 84, "right");

    titulo("VALOR DO PIS", 441, 270, 47);
    campo(danfe.Impostos.ValorDoPis, 441, 278, 47, "right");
    ...
    campo(danfe.OutrasDespesas, 265, 298, 84, "right");

    titulo("VALOR TOTAL DO IPI", 353, 290, 84);
    campo(danfe.Impostos.ValorTotalDoIpi, 353, 298, 84, "right");
    ...
    campo(danfe.Impostos.ValorDaCofins, 440.5, 298, 47, "right");
    ...
    campo(danfe.Transportador.Nome, 1.5, 329, 166.5, "left");

    titulo("FRETE POR CONTA", 171.5, 321, 85);
    campo(danfe.ModalidadeDoFrete, 171.5, 329, 85);

    titulo("CÓDIGO ANTT", 259.5, 321, 84);
    campo(danfe.Transportador.CodigoAntt, 259.5, 329, 84);

    titulo("PLACA DO VEÍCULO", 347.5, 321, 84);
    campo(danfe.Transportador.PlacaDoVeiculo, 347.5, 329, 84);

    titulo("UF", 435.5, 321, 19.5);
    campo(danfe.Transportador.UfDaPlacaDoVeiculo, 435.5, 329, 19.5);

    titulo("CNPJ / CPF", 458, 321, 126.5);
    campo(danfe.Transportador.RegistroNacional, 458, 329, 126.5);

    titulo("ENDEREÇO", 1.5, 341, 254);
    campo(
      danfe.Transportador.Endereco.Logradouro,
      1.5,
      349,
      254,
      "left",
    ...
    campo(danfe.Transportador.Endereco.Municipio, 259.5, 349, 172);

    titulo("UF", 435.5, 341, 19.5);
    campo(danfe.Transportador.Endereco.Uf, 435.5, 349, 19.5);

    titulo("INSCRIÇÃO ESTADUAL", 458, 341, 126.5);
    campo(danfe.Transportador.InscricaoEstadual, 458, 349, 126.5);

    titulo("QUANTIDADE", 1.5, 361, 56.5);
    campo(danfe.Volumes.Quantidade, 1.5, 369, 56.5);

    titulo("ESPÉCIE", 60.8, 361, 96);
    campo(danfe.Volumes.Especie, 60.8, 369, 96);

    titulo("MARCA", 160, 361, 96);
    campo(danfe.Volumes.Marca, 160, 369, 96);

    titulo("NUMERAÇÃO", 259.5, 361, 96);
    campo(danfe.Volumes.Numeracao, 259.5, 369, 96);

    titulo("PESO BRUTO", 358.5, 361, 112.5);
    campo(danfe.Volumes.PesoBruto, 358.5, 369, 112.5);

    titulo("PESO LÍQUIDO", 474.5, 361, 110.5);
    campo(danfe.Volumes.PesoLiquido, 474.5, 369, 110.5);

    var alturaDaSecaoDosItens = 383,
      alturaDosTitulosDosItens;

    alturaDosTitulosDosItens = alturaDaSecaoDosItens + 14;
  ...
  danfe.Item.forEach(function (item, index) {
    //for (i = 0; i < danfe.Item.length; i++) {
